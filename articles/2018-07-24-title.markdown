...
* `jobs`: These are specific tasks you want CircleCI to run upon checking in of code i.e. everytime a build is triggered. Jobs let's just similar / related tasks together. In your `config.yml`, you defined one job, `build`. Under the `build` job, you specified the docker image to be used to run the build, then select the working directory. The `steps` section of the job is where you define specific task to perform everytime build is triggered. Below is the analysis of the `steps` section;
...
* `workflows`: A workflow is a set of rules for defining a collection of jobs and their run order. Workflows support complex job orchestration using a simple set of configuration keys to help you resolve failures sooner. In this article, we defined `build-deploy` rule to run your `build` job and `deploy` only if build passes. The `deploy` job would be added later on in this article.
...
#### Workflow
Did you notice the changes we made to the workflow? Basically, what we have done is make the `deploy` job dependent on the `build` so it only runs when the build runs successfully.
